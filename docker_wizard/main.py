...
from docker_wizard.changelog_generator import generate_change_log
from docker_wizard.dependency_upgrader import (
    _MAJOR,
    _MINOR,
    _PATCH,
    _get_dependency_upper_bound_for_runtime_upgrade,
)
from docker_wizard.package_report import (
    generate_package_size_report,
    generate_package_staleness_report,
)
from docker_wizard.release_notes_generator import generate_release_notes
from docker_wizard.utils import (
...
def build_images(args):
    ...
    image_ids, image_versions = _build_local_images(
        target_version, args.target_ecr_repo, image_config, args.force, args.skip_tests
    )
    ...
    if not args.skip_tests:
        ...
        _test_local_images(image_ids, image_config, args.target_patch_version)
    else:
...
def _test_local_images(image_ids_to_test: list[str], image_config: list[dict], target_version: str):
    assert len(image_ids_to_test) == len(image_config)
    exit_codes = []
    image_ids = []
    for image_id, config in zip(image_ids_to_test, image_config):
...
def _build_local_images(
    target_version: Version, target_ecr_repo_list: list[str], image_config: list[dict], force: bool, skip_tests=False
    ...
    for image_generator_config in image_config:
        ...
        image_tags_to_apply = [i + "-" + config["image_tag_generator_suffix"] for i in version_tags_to_apply]
        ...
        image.tag("localhost/sagemaker-distribution", str(target_version) + "-" + config["image_tag_generator_suffix"])
...
def _generate_python_package_metadata(target_version_dir, image, image_config):
    try:
        pkg_metadata = _docker_client.containers.run(
            ...
            detach=False,
            auto_remove=True,
            command=f"git clone https://github.com/aws-tianquaw/sagemaker-distribution.git && cd sagemaker-distribution && conda env update -f environment.yml && docker-wizard get-conda-package-metadata",
        )
...
def get_arg_parser():
    ...
    for p in [create_major_version_parser, create_minor_version_parser, create_patch_version_parser]:
        ...
        p.add_argument(
            "--image-config-file",
            required=True,
            help="A json file contains the docker image generator configuation.",
        )
        p.add_argument(
    ...
    build_image_parser.add_argument(
        "--target-patch-build-artifacts-dir",
        required=True,
        help="Specify the directory holding the build artifacts for target image needs to be built.",
    )
    build_image_parser.add_argument(
        "--image-config-file",
        required=True,
